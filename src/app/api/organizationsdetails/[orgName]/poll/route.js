...
/**
 * POST /api/organizationsdetails/[orgName]/poll
 * Fetch and analyze issues for an organization
 */
export async function POST(request, { params }) {
  try {
    ...
    const reposResponse = await fetch(
      `https://api.github.com/orgs/${organization.orgName}/repos?per_page=100`,
      {
        headers: {
    ...
    // For each repository, fetch unassigned issues
    for (const repo of repos) {
      const issuesResponse = await fetch(
        `https://api.github.com/repos/${organization.orgName}/${repo.name}/issues?state=open&per_page=100`,
        {
          headers: {
            Authorization: `token ${GITHUB_TOKEN}`,
            Accept: 'application/vnd.github.v3+json',
      ...
      for (const ghIssue of unassignedIssues) {
        ...
        if (!existingIssue) {
          ...
          if (matchScore >= 75) {
            await autoCommentOnIssue(newIssue, user, ghIssue, organization.orgName, repo.name);
          }

          // Add to tracked issues
          organization.trackedIssues.push(newIssue._id);
        }
      }
    }

    // Update organization
    organization.lastPolled = new Date();
    organization.totalIssues = organization.trackedIssues.length;
    ...
    return NextResponse.json({
      ...
      totalTracked: organization.totalIssues,
    });
  } catch (error) {
    console.error('Polling error:', error);
    return NextResponse.json(
      { error: 'Failed to poll organization', details: error.message },
      { status: 500 }
    );
  }
}
 ...
 */
async function autoCommentOnIssue(issue, user, ghIssue, orgName, repoName) {
  try {
...
Keep it under 200 words.`;
    ...
    const commentResponse = await fetch(
      `https://api.github.com/repos/${orgName}/${repoName}/issues/${ghIssue.number}/comments`,
      {
        method: 'POST',
        headers: {
          Authorization: `token ${GITHUB_TOKEN}`,
